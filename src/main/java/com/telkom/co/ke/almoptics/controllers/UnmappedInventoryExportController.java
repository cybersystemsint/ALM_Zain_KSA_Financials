...
public class UnmappedInventoryExportController {
    ...
    private static final String[] ACTIVE_COLUMNS = {
        "rowNumber","nodeId", "nodeName", "nodeType", "serialNumber",
    "model", "partNumber", "siteId", "manufacturer", "description", "manufacturingDate",
    "installationDate", "assetInsertionDate"
    };
    private static final String[] ACTIVE_HEADERS = {
        "Sequence Number", "Node ID", "Node Name", "Node Type", "Serial Number",
    "Model", "Part Number", "Site ID", "Manufacturer", "Description", "Manufacturing Date",
    "Installation Date", "Asset Insertion Date"
    };
    
    private static final String[] PASSIVE_COLUMNS = {
       "rowNumber","inventoryId","objectId","elementType","parentName",
    ...
    "locationSubType","locationClassification","itemClassification","itemClassification2","notes","prPoNo"
    };
    ...
    "Location Sub Type","Location Classification","Item Classification","Item Classification 2","Notes","PR/PO No"
    };
    
    private static final String[] IT_COLUMNS = {
        "rowNumber", "objectId","siteId","hostSerialNumber","inventoryTypeId","inventoryType",
    ...
    public StreamingResponseBody exportActive(
        ...
        logger.info("POST /exporting/unmapped-active started, request={}", req);
        ...
        return outputStream -> exportService.exportToExcel(outputStream, "tb_unmapped_active_inventory", ACTIVE_COLUMNS,ACTIVE_HEADERS, req);
    }
    ...
    public StreamingResponseBody exportPassive(
            @RequestBody InventoryRequest req,
            HttpServletResponse response) {
        logger.info("POST /exporting/unmapped-passive started, request={}", req);
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setHeader("Content-Disposition", "attachment; filename=UnmappedPassive_" + System.currentTimeMillis() + ".xlsx");
        response.setHeader("Cache-Control", "no-cache");
        return outputStream -> exportService.exportToExcel(outputStream, "tb_unmapped_passive_inventory", PASSIVE_COLUMNS,PASSIVE_HEADERS, req);
    }
    ...
    public StreamingResponseBody exportIT(
            @RequestBody InventoryRequest req,
            HttpServletResponse response) {
        logger.info("POST /exporting/unmapped-it started, request={}", req);
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setHeader("Content-Disposition", "attachment; filename=UnmappedIT_" + System.currentTimeMillis() + ".xlsx");
        response.setHeader("Cache-Control", "no-cache");
        return outputStream -> exportService.exportToExcel(outputStream, "tb_unmapped_IT_Inventory", IT_COLUMNS, IT_HEADERS,  req);
    }
}
