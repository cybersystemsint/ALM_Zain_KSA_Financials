...
public class FarReportServiceImpl implements FarReportService {
	...
	@Autowired
	public FarReportServiceImpl(FarReportRepository farReportRepository, JdbcTemplate jdbcTemplate) {
	...
	@Override
	public FarReport save(FarReport boards) {
		return farReportRepository.save(boards);
	}

	@Override
	public List<FarReport> findByAssetId(String paramString) {
		return farReportRepository.findByAssetId(paramString);
	}

	@Override
	public List<FarReport> findAll(int page, int size) {
		Pageable pageable = PageRequest.of(page, size);
		Page<FarReport> pageResult = farReportRepository.findAll(pageable);
	...
	private static final String[] COLUMNS = { "recordNo", "recordDatetime", "book", "assetId", "quantity",
			"description", "asset_type", "creationDate", "serialNumber", "tagNumber", "picStatus", "picDate",
			"cipDeliveryDate", "linkId", "acceptanceNumber", "depreciateFlag", "cipEu", "invoiceNumber", "poNumber",
			"poLineNumber", "uplLine", "transferToNewFar", "assetStatus", "value", "partNumber", "vendorName",
			"vendorNumber", "mergedCode", "costAccount", "accumulatedDepreAccount", "cipCostAccount",
			"expenseCostCenter", "expenseAccount", "Life", "datePlacedInService", "cost", "nbv", "depreciationAmount",
			"ytdDepreciation", "depreciationReserve", "salvageValue", "category", "categoryDescription",
			"locationSegment1", "locationSegment2", "locationSegment3", "locationSegment4", "locations",
			"sequenceNumber", "createdBy", "createdDate", "updatedBy", "updatedDate", "monthlyDepreciationAmt",
			"accumulatedDepreciationAmt", "depreciationDate", "netCost", "statusFlag", "changedBy", "insertedBy",
